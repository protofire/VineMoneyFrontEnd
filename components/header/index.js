...
export default function Header(props) {
    ...
    const { account, setAccount, ethersProvider, setEthersProvider, setSigner, setSignInAuth, setSignInAuthToken, infuraRPC,
        idovestingQuery, idovestingMain, sapphireProviderSigner, troveManagerGetters, debtToken, signInAuth,
        currentState, setCurrentState, setCurrentWaitInfo, formatNum, totalTvl } = useContext(UserContext);
    ...
    const queryData = async () => {
        ...
        const airDrop = new BigNumber(unlockingInfo.airdrop._hex).div(1e18).toFixed();
        setAirDrop(Number(airDrop));
        const unlockingStartTime = await idovestingQuery.unlockingStartTime();
        var timestamp = Date.parse(new Date()) / 1000;
        if (Number(airDrop) > 1 && unlockingInfo.isClaimed == false && timestamp > Number(unlockingStartTime._hex)) {
            setShowClaim(true);
        } else {
            setShowClaim(false)
        }
    ...
    useEffect(() => {
        if (lastUnlockingTime <= 0) {
            ...
            const claimed = (Number(lastUnlockingTime) - Number(unlockingStartTime)) / (180 * 86400) * Number(totalLocked);
            setClaimed(claimed);
    ...
    const ClaimIdo = async () => {
        if (showClaim) {
            try {
                ...
                setCurrentWaitInfo({ type: "loading", info: "Claim " + Number(airDrop.toFixed(4)).toLocaleString() + " $VINE" })
    ...
    const Claim = async () => {
        ...
        try {
            ...
            setCurrentWaitInfo({ type: "loading", info: "Claim " + formatNum(unlockableAmount) + " $VINE" })
            setCurrentState(true);
            const result = await idoTx.wait();
            setCurrentState(false);
            if (result.status === 0) {
            ...
            <div className={styles.head}>
                <div className={styles.headMain} id='vine'>
                    <Link href="/" className={styles.logo}>
                        ...
                        VINE
                        {type == "dapp" ? <span className='tvl'>TVL:${formatNum(totalTvl)}</span> : null}
                    </Link>
                    {type == "dapp" ?
                        <div className={styles.dappList}>
                            <Link className={dappMenu == "Vault" ? `${styles.active}` : null} href="/Vault" rel="nofollow noopener noreferrer"><span>Vault</span></Link>
                            <Link className={dappMenu == "Mint" ? `${styles.active}` : null} href="/Mint" rel="nofollow noopener noreferrer"><span>Mint vUSD</span></Link>
                            <Link className={dappMenu == "Earn" ? `${styles.active}` : null} href="/Earn" rel="nofollow noopener noreferrer"><span>Earn</span></Link>
                            <Link className={dappMenu == "Reward" ? `${styles.active}` : null} href="/Reward" rel="nofollow noopener noreferrer"><span>Reward</span></Link>
                            <Link className={dappMenu == "Lock" ? `${styles.active}` : null} href="/Lock" rel="nofollow noopener noreferrer"><span>Lock</span></Link>
                            <Link className={dappMenu == "Redeem" ? `${styles.active}` : null} href="/Redeem" rel="nofollow noopener noreferrer"><span>Redeem</span></Link>
                            <Link className={dappMenu == "Vote" ? `${styles.active}` : null} href="/Vote" rel="nofollow noopener noreferrer"><span>Vote</span></Link>
                        </div>
                    ...
                    <div className={styles.menuList}>
                        {menu == "Home" ? parseInt(new Date().getTime() / 1000) < 1709730000 ? (
                            ...
                            : <div div className='button'>
                                <Link href="/Vault"><span>Launch App</span></Link>
                            </div>
                            : null}
                        <>
                            <div className='h5None'>
                                {wallet ?
                                    <div style={{ "display": "flex", "gap": "5px" }}>
                                        <div style={{ "display": "flex", "gap": "5px" }}>
                                            <div className='account'>{account.slice(0, 5) + ".." + account.slice(-5)}</div>
                                            <div className='button h5None' style={{ "minWidth": "auto" }}
                                                onClick={() => disconnect(wallet)}
                                            >
                                                Disconnect
                                            </div>
                                        </div>
                                        {showClaim || unlockableAmount ? <div className={`${"button"} ${styles.faucet}`} style={{ "minWidth": "auto" }}>
                ...
                !open ? type == "dapp" ?
                    <div className={styles.h5Block}>
                        <div className={styles.h5Item}>
                            <Link href="/Vault" rel="nofollow noopener noreferrer"><span>Vault</span></Link>
                        </div>
                        <div className={styles.h5Item}>
                            <Link href="/Mint" rel="nofollow noopener noreferrer"><span>Mint vUSD</span></Link>
                        </div>
                        <div className={styles.h5Item}>
                            <Link href="/Earn" rel="nofollow noopener noreferrer"><span>Earn</span></Link>
                        </div>
                        <div className={styles.h5Item}>
                            <Link href="/Reward" rel="nofollow noopener noreferrer"><span>Reward</span></Link>
                        </div>
                        <div className={styles.h5Item}>
                            <Link href="/Lock" rel="nofollow noopener noreferrer"><span>Lock</span></Link>
                        </div>
                        <div className={styles.h5Item}>
                            <Link href="/Redeem" rel="nofollow noopener noreferrer"><span>Redeem</span></Link>
                        </div>
                        <div className={styles.h5Item}>
                            <Link href="/Vote" rel="nofollow noopener noreferrer"><span>Vote</span></Link>
                        </div>
                        <div className='h5user'>
                            {wallet ?
                                <>
                                    <div style={{ "display": "flex", "gap": "5px" }}>
                                        <div className='account'>{account.slice(0, 5) + ".." + account.slice(-5)}</div>
                                        <div className='button' style={{ "minWidth": "auto" }}
                                            onClick={() => disconnect(wallet)}
                                        >
                                            Disconnect
                                        </div>
                                        {showClaim || unlockableAmount ? <div className={`${"button"} ${styles.claim}`} style={{ "minWidth": "auto" }}>
                                            <span onClick={() => ClaimIdo()}>Claim</span>
                                        </div> : null}
                                    </div>
                    ...
                    <div className={styles.h5Block}>
                        ...
                        <div className={styles.h5Item} onClick={() => goMenu_h5('works')}>How it works</div>
                        <div className={styles.h5Item}>
                            <Link target="_blank" href="https://vine-money.gitbook.io/vine-money/" rel="nofollow noopener noreferrer"><span>Docs</span></Link>
                        </div>
                        <div className={`${styles.h5Item}`}>
                            ...
                            <div className={styles.socials}>
                                ...
                                <div><Link target="_blank" href="https://medium.com/@vine_money" rel="nofollow noopener noreferrer">Medium</Link></div>
                            </div>
                        </div>
                        <div className={styles.h5Item} onClick={() => goMenu_h5('faq')}>FAQ</div>
                        <div className={styles.h5Item}>
                            <Link target="_blank" href="/Vine_Money_Disclaimer.pdf" rel="nofollow noopener noreferrer"><span>Disclaimer</span></Link>
                        </div>
                                ...
                                <>
                                    <div style={{ "display": "flex", "gap": "5px" }}>
                                        ...
                                        {showClaim || unlockableAmount ?
                                            <div className={`${"button"} ${styles.claim}`} style={{ "minWidth": "auto" }}>
                                                <span onClick={() => ClaimIdo()}>Claim</span>
                                            </div>
                                            : null}
                                    </div>
            ...
            {showSignIn
                ? <div className="promptBox">
                    <div className="boxMain">
                        <div className="boxInfo">Vine is the first and only encrypted DeFi protocol for Web3 that provides intelligent privacy features. Only your personal signature grants access to individual data. To streamline the signing process and enhance user experience, you are required to use EIP-712 to "sign in" once per day.
                        </div>
                        <div className='button rightAngle' onClick={() => signIn()}>sign in</div>
                    </div>
                </div> : null}
                ...
                ? <div className="promptBox">
                    <div className="boxMain">
                        <div className="boxInfo">
                            Please sign in your wallet's pop-up to allow Vine to access your vUSD balance.
                        </div>
                        <div className='button rightAngle' onClick={() => signInToken()}>sign in</div>
                    </div>
                </div> : null}
                ...
                <div className='infoTip'>
                    <div className='info infoNoPadding'>
                        <div className='infoTitle'>
                            ...
                            <div className="close">
                                <img src='/icon/close.svg' alt='icon' onClick={() => setshowClaim2(false)} />
                            </div>
                        </div>
                        <div className="data">
                            ...
                            <div className={unlockableAmount > 0.001 ? 'button rightAngle height' : 'button rightAngle height disable noactive'} style={{ "marginTop": "10px" }} onClick={() => Claim()}>Claim</div>
                        </div>
                    </div>
                </div>
                : null
