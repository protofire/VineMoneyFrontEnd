...
export default function Lock() {
    const { vineTokenQuery, tokenLockerMain, tokenLocker, tokenLockerQuery, boostCalculatorQuery,
        account, currentState, setCurrentState, setCurrentWaitInfo, boost, formatNum, vinePrice
    ...
    const queryData = async () => {
        if (account) {
            ...
            setAccountLock(Number(new BigNumber(accountBalances.locked._hex).toFixed()));
            setAccountUnLock(Number(new BigNumber(accountBalances.unlocked._hex).toFixed()));
    ...
    useEffect(() => {
        if (Number(amount) && currentValue) {
            ...
            const futureDate = new Date(currentDate.getTime() + d * 24 * 60 * 60 * 1000);
    ...
    const changeClaimAmount = async (e) => {
        const value = Number(e.target.value);
        const withdrawWithPenaltyAmounts = await tokenLockerQuery.getWithdrawWithPenaltyAmounts(account, value);
        setAmountWithdrawn(Number(withdrawWithPenaltyAmounts.amountWithdrawn._hex) / 1e18);
        setPenaltyAmountPaid(Number(withdrawWithPenaltyAmounts.penaltyAmountPaid._hex) / 1e18)
    ...
    const Lock = async () => {
        ...
        try {
            ...
            setCurrentWaitInfo({ type: "loading", info: "Lock " + Number(amount.toFixed(4)).toLocaleString() + " $VINE" })
    ...
    const EarlyUnlock = async () => {
        ...
        try {
            const lockTx = await tokenLockerMain.withdrawWithPenalty(Math.floor(claimAmount));
            setCurrentWaitInfo({ type: "loading", info: "Early Unlock " + Math.floor(claimAmount) + " $VINE" })
            setCurrentState(true);
            const result = await lockTx.wait();
            setCurrentState(false);
            if (result.status === 0) {
                        ...
                        <div className={styles.dataInfo2}>
                            ...
                            <div className={styles.value}>
                                ...
                                <div style={{ "display": "flex" }}>
                                    <img style={{ "width": "26px" }} src='/dapp/vine.svg' alt='vUSD' />
                                    <p>{formatNum(accountLock + accountUnLock)}</p>
                                </div>
                                <span className={styles.span}>≈ ${formatNum((accountLock + accountUnLock) * vinePrice)}</span>
                                {accountLock + accountUnLock > 0
                                    ?
                                    <div className='button_border' style={{ "padding": "5px", "marginTop": "5px", "lineHeight": "18px" }} onClick={() => changeShowUnlock()}>Claim</div>
                                    : null}
                            </div>
                        </div>
                        <div className={styles.dataInfo2}>
                    ...
                    <div className={styles.lockMain}>
                        <div className={styles.left}>
                            <div className={styles.title}>
                                <p>Lock $VINE</p>
                                <span>Lock for up to 52 weeks. Locked $VINE gives lock weight and allows boosted claiming.</span>
                            </div>
                            <div className={styles.enterAmount}>
                                <div className={styles.miniTitle}>
                                    <span>Enter amount</span>
                                    <span style={{ "fontSize": "12px" }}>Balance {Number(Number(balance).toFixed(2)).toLocaleString()}$VINE</span>
                                </div>
                                <div className='inputTxt3'>
                                    <input type='number' placeholder='0' onWheel={(e) => e.target.blur()} id="amount" onKeyDown={onKeyDown.bind(this)} onChange={changeAmount.bind(this)} value={amount} ></input>
                                    <span>VINE</span>
                                </div>
                                <div className="changeBalance">
                                    ...
                                    <span onClick={() => changeVaule(1)} style={{ "border": "none" }}>Max</span>
                                </div>
                                <div className={styles.progressBar}>
                                    <Slider min={2} max={52} marks={marks} onChange={log} defaultValue={26}
                            ...
                            <div className={styles.button}>
                                <div className={!amount ? "button rightAngle height disable" : 'button rightAngle height'} onClick={() => Lock()}>LOCK</div>
                            </div>
                            <div className={styles.data}>
                                    ...
                                    <div>
                                        ...
                                        {Number(amount) ?
                                            <>
                                                ...
                                                <span>{formatNum(Number(accountWeight) + Number(amount) * currentValue)}</span>
                                            </>
                                            : null
                                        }
                                    </div>
                                ...
                                <div className={styles.dataItem}>
                                    <p>Total locked $VINE</p>
                                    <div>
                                        {formatNum(accountLock + accountUnLock)}
                                        {Number(amount) ?
                                            <>
                                                <img src='/dapp/right.svg' alt='icon' />
                                                <span>{formatNum(Number(accountLock + accountUnLock) + Number(amount))}</span>
                                            </>
                                            : null
                                        }
                                    </div>
                        ...
                        <div className={styles.right}>
                            <div className={styles.main}>
                                ...
                                <div>
                                    {!isLocks
                                        ? <span className={Number(accountLock) > 0 ? 'button_border' : 'button_border disable'} onClick={() => enableAutoLock()}>
                                            Enable auto lock
                                        </span>
                                        :
                                        <span className='button_border' onClick={() => disableAutoLock()}>
                                            Disable auto lock
                                        </span>
                                    }
                                </div>
                            ...
                            {!isLocks ?
                                Number(accountLock) > 0 ? <div className={styles.span}>
                                    This allows you to combine all your existing locks in to one lock that is locked in perpetuity for 52 weeks. This ensures your vote-weight does not decrease week on week and saves you spending gas on relocking.
                                </div> : <div className={styles.span}>
                                    No active locks, lock some $VINE to gain Lock Weight
                                </div>
                                : null
                            }
                ...
                <div className='info infoNoPadding'>
                    ...
                    <div className="data">
                        <div className="dataItem">
                            <p>Locked </p>
                            <span>{formatNum(accountLock)} $VINE</span>
                        </div>
                        <div className="dataItem">
                            <p>Unlocked </p>
                            <span>{formatNum(accountUnLock)} $VINE</span>
                        </div>
                        <div className='inputTxt2'>
                            <input type='number' placeholder='0' onWheel={(e) => e.target.blur()} id="claimAmount" onKeyDown={onKeyDown.bind(this)} onChange={changeClaimAmount.bind(this)} value={claimAmount} ></input>
                            <span>$VINE</span>
                        </div>
                        {claimAmount > accountUnLock ?
                            <p style={{ "textAlign": "center", "margin": "10px 0 20px", "fontSize": "14px" }}>Early Unlock Penalty: <span style={{ "color": "#509D7B" }}>{formatNum(penaltyAmountPaid)}$VINE</span></p> : null}
                        <div className="button rightAngle" style={{ "marginTop": "20px" }} onClick={() => EarlyUnlock()}>Claim</div>
                    </div>
                </div>
            </div> : null}
            {/* {showUnlock ? <div className='infoTip'>
