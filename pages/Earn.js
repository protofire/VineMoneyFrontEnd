...
export default function Earn() {
    ...
    const { currentState, setCurrentState, setCurrentWaitInfo,
        account, vineLpTokenPoolMain, vineLpTokenPoolQuery, mockLpMain, mockLpQuery, VineLpTokenPool
        , lpPrice, stabilityPoolMain, stabilityPoolQuery, vUSDbalance, rosePrice, totalRose,
        debtTokenQuery, debt, troveManagerQuery, vinePrice, boost, formatNum,
        VUSDUSDCLPQuery, usdcPoolQuery, VUSDUSDCLPMain, usdcPoolMain, usdcPool, VUSDUSDCLP,
    ...
    const queryData = async () => {
        if (account) {
            ...
            //Claim
            const getDepositorCollateralGain = await stabilityPoolQuery.getDepositorCollateralGain(account);
            setDepositorCollateralGain((new BigNumber(getDepositorCollateralGain[0]._hex).div(1e18)).toFixed());
        ...
        setBaseAPR1(Number(baseAPR1._hex) / 1e18);
    ...
    useEffect(() => {
        ...
        const vUSDBaseApr2 = baseAPR2 * 86400 * 365 * vinePrice * 100 / (Number(mockLpBanace) * lpPrice);
        setvUSDBaseApr2(vUSDBaseApr2);
        const vUSDBaseApr4 = baseAPR4 * 86400 * 365 * vinePrice * 100 / (Number(VUSDLpBanace) * 2);
        setvUSDBaseApr4(vUSDBaseApr4);
        const vUSDBaseApr3 = baseAPR3 * 86400 * 365 * vinePrice * 100 / (Number(stabilityPoolBanace) + (Number(totalRose) * rosePrice));
        setvUSDBaseApr3(vUSDBaseApr3);
    }, [vUSDCirc, baseAPR1, baseAPR2, baseAPR3, vinePrice, mockLpBanace, lpPrice, stabilityPoolBanace, totalRose, rosePrice, baseAPR4, VUSDLpBanace])
    ...
    }, [typeName, changeType, stakeLpBalance, unStakeLpBalance, accountDeposits, stakeLpBalance2, unStakeLpBalance2, depositorCollateralGain])
    ...
    }, [typeName, mockLpBanace, lpPrice, stabilityPoolBanace, rosePrice, totalRose, VUSDLpBanace])


    const stakeApprove = async () => {
        try {
            const tx = await mockLpMain.approve(VineLpTokenPool, new BigNumber(amount).multipliedBy(1e18).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "Approve " + Number(amount.toFixed(4)).toLocaleString() + " LP" })
    ...
    const stakeLp = async () => {
        try {
            const tx = await vineLpTokenPoolMain.deposit(account, new BigNumber(amount).multipliedBy(1e18).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "Stake " + Number(amount.toFixed(4)).toLocaleString() + " LP" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
            if (result.status === 0) {
    ...
    const unStakeLp = async () => {
        try {
            const tx = await vineLpTokenPoolMain.withdraw(account, new BigNumber(amount).multipliedBy(1e18).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "UnStake " + Number(amount.toFixed(4)).toLocaleString() + " LP" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
            if (result.status === 0) {
    ...
    const stakeApprove2 = async () => {
        try {
            const tx = await VUSDUSDCLPMain.approve(usdcPool, new BigNumber(amount).multipliedBy(1e18).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "Approve " + Number(amount.toFixed(4)).toLocaleString() + " LP" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
    ...
    const stakeLp2 = async () => {
        try {
            const tx = await usdcPoolMain.deposit(account, new BigNumber(amount).multipliedBy(1e6).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "Stake " + Number(amount.toFixed(4)).toLocaleString() + " LP" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
    ...
    const unStakeLp2 = async () => {
        try {
            const tx = await usdcPoolMain.withdraw(account, new BigNumber(amount).multipliedBy(1e6).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "UnStake " + Number(amount.toFixed(4)).toLocaleString() + " LP" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
    ...
    const Deposit = async () => {
        try {
            const tx = await stabilityPoolMain.provideToSP(new BigNumber(amount).multipliedBy(1e18).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "Deposit " + Number(amount.toFixed(4)).toLocaleString() + " vUSD" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
            if (result.status === 0) {
    ...
    const Withdraw = async () => {
        try {
            const tx = await stabilityPoolMain.withdrawFromSP(new BigNumber(amount).multipliedBy(1e18).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "Withdraw " + Number(amount.toFixed(4)).toLocaleString() + " vUSD" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
            if (result.status === 0) {
    ...
    const Claim = async () => {
        try {
            ...
            setCurrentWaitInfo({ type: "loading", info: "Claim " + Number(amount.toFixed(4)).toLocaleString() + " vUSD" })
            setCurrentState(true);
            const result = await tx.wait();
            setCurrentState(false);
            if (result.status === 0) {
    ...
    return (
        <>
            ...
            <div className='dappBg'>
                {!showEarnMain ? <div className={`${styles.Earn} ${"dappMain2"}`} >
                    ...
                    <div className={styles.earnMain}>
                        <div className={styles.earnInfo}>
                            <p className={styles.earnTip}>Lock $VINE to boost your APR to {boost}x.</p>
                            ...
                            <div className={styles.button} onClick={() => router.push("/Mint")}>
                        ...
                        <div className={styles.earnInfo}>
                            <p className={styles.earnTip}>Lock $VINE to boost your APR to {boost}x.</p>
                            <div className={styles.CoinType}>
                                <img style={{ "width": "65px" }} src='/dapp/vineArose.svg' alt='vUSD' />
                                VINE/ROSE LP
                            </div>
                            <div className={styles.data}>
                                <div className={styles.dataItem}>
                            ...
                            <div className={styles.button} onClick={() => changeManage("VINE/ROSE LP")}>
                                <div className='button rightAngle height'>Manage</div>
                            </div>
                        </div>
                        <div className={styles.earnInfo}>
                            <p className={styles.earnTip}>Lock $VINE to boost your APR to {boost}x.</p>
                            <div className={styles.CoinType}>
                                <img style={{ "width": "65px" }} src='/dapp/usdc.svg' alt='icon' />
                                vUSD/USDC LP
                            </div>
                            <div className={styles.data}>
                                <div className={styles.dataItem}>
                            ...
                            <div className={styles.button} onClick={() => changeManage("vUSD/USDC LP")}>
                                <div className='button rightAngle height'>Manage</div>
                            </div>
                        </div>
                        <div className={styles.earnInfo}>
                            ...
                            <div className={styles.data}>
                                ...
                                <div className={styles.dataItem}>
                                    <p>TVL</p>
                                    <span>${formatNum(Number(stabilityPoolBanace) + (Number(totalRose) * rosePrice))}</span>
                                </div>
                                <div className={styles.dataItem}>
                                    <p>Base APR</p>
                            ...
                            <div className={styles.button} onClick={() => changeManage("Stability Pool")}>
                                <div className='button rightAngle height'>Manage</div>
                            </div>
                        </div>
                    </div>
                ...
                {showEarnMain ?
                    <div className={`${styles.Earn2} ${"dappMain"}`}>
                        ...
                        <div className={styles.earnMain}>
                            <div className={styles.title}><img src='/dapp/vUSD.svg' alt='vUSD' />{typeName}</div>
                            <div className={styles.dataInfo}>
                                <div className={styles.value}>
                                    <span>APR</span>
                                    <div>
                                        <p>{typeName == "VINE/ROSE LP" ? formatNum(vUSDBaseApr2 * boost) : typeName == "vUSD/USDC LP" ? formatNum(vUSDBaseApr4 * boost) : formatNum(vUSDBaseApr3 * boost)} ({boost}x)</p>
                                    </div>
                                </div>
                                <div className={styles.value}>
                                    <span>Pool TVL</span>
                                    ...
                                    <div className={styles.manageDesc}>
                                        ...
                                        <div>
                                            ...
                                            {typeName == "Stability Pool" ?
                                                <Link target='_blank' href='https://vine-money.gitbook.io/vine-money/protocol/stability-pool-and-liquidations' rel='nofollow noopener noreferrer'>
                                                    <span style={{ "color": "#57CC99" }}> Read more.</span>
                                                </Link>
                                                : null}</div>

                                    </div>
                                    <div className={styles.opType} style={typeName == "Stability Pool" ? { "gridTemplateColumns": "1fr 1fr 1fr" } : null}>
                                        {typeName == "Mint vUSD" ? <><span className={changeType == "Mint" ? `${styles.active}` : null} onClick={() => changeTypeCoin("Mint", "vUSD")} style={{ "display": "none" }}>Mint</span></> : null}
                                        {typeName == "VINE/ROSE LP" || typeName == "vUSD/USDC LP" ? <><span className={changeType == "Stake" ? `${styles.active}` : null} onClick={() => changeTypeCoin("Stake", "LP")}>Stake</span><span className={changeType == "UnStake" ? `${styles.active}` : null} onClick={() => changeTypeCoin("UnStake", "LP")}>UnStake</span></> : null}
                                        {typeName == "Stability Pool" ? <>
                                            <span className={changeType == "Deposit" ? `${styles.active}` : null} onClick={() => changeTypeCoin("Deposit", "vUSD")}>Deposit</span>
                                            <span className={changeType == "Withdraw" ? `${styles.active}` : null} onClick={() => changeTypeCoin("Withdraw", "vUSD")}>Withdraw</span>
                                            <span className={changeType == "Claim" ? `${styles.active}` : null} onClick={() => changeTypeCoin("Claim", "$ROSE")}>Reward</span>
                                        </> : null}
                                    </div>
                                    <div className='balance'>
                                        <p>{changeType} {coin}</p>
                                        <span>{changeType == "Claim" ? null : "MAX "} {Number(maxBalance.toFixed(4)).toLocaleString()} {coin}</span>
                                    </div>
                                    {changeType == "Claim" ? null : <>
                                        <div className='inputTxt2'>
                                            <div>
                                                <input type="number" placeholder='0' onWheel={(e) => e.target.blur()} id="amount" onKeyDown={onKeyDown.bind(this)} onChange={changeAmount.bind(this)} value={amount} />
                                                <span className='font_12_gray'>≈$
                                                    {typeName == "VINE/ROSE LP"
                                                        ? formatNum(Number(amount) * lpPrice)
                                                        : typeName == "vUSD/USDC LP"
                                                            ? formatNum(Number(amount) * (tvl / USDCtotalSupply))
                                                            : formatNum(Number(amount))}</span>
                                            </div>
                                        ...
                                        <div className="changeBalance" style={{ "marginTop": "12px" }}>
                                            ...
                                            <span onClick={() => changeAmountVaule(1)} style={{ "border": "none" }}>Max</span>
                                        </div></>}
                                </div>
                                <div className={styles.button}>
                                    {changeType == "Claim"
                                        ? <div className={!maxBalance ? "button rightAngle height disable" : 'button rightAngle height'} onClick={() => Operate()}>{buttonName}</div>
                                        : <div className={!amount ? "button rightAngle height disable" : 'button rightAngle height'} onClick={() => Operate()}>{buttonName}</div>
                                    }
                                </div>
                            </div>
