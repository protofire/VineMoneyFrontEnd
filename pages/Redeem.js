...
export default function Redeem() {
    const { account, currentState, setCurrentState, setCurrentWaitInfo,
        vUSDbalance, multiCollateralHintHelpersQuery, troveManager, rosePrice,
        sortedTrovesToken, troveManagerMain, troveManagerQuery, formatNum } = useContext(UserContext);
    ...
    const [expectedCollateralReceived, setExpectedCollateralReceived] = useState(0);
    useEffect(() => {
        if (amount) {
            setFeeAmount(Number(amount) / rosePrice * fee);
            setExpectedCollateralReceived(Number(amount) / rosePrice - (Number(amount) / rosePrice * fee))
        } else {
            setFeeAmount(0);
    ...
    const redeemCollateral = async () => {
        ...
        const redemptionHints = await multiCollateralHintHelpersQuery.getRedemptionHints(
            troveManager,
        ...
        const status = Number(redemptionHints.truncatedDebtAmount._hex) == Number(new BigNumber(amount).multipliedBy(1e18).toFixed()) ? 1 : 0;
        // console.log(Number(redemptionHints.partialRedemptionHintNICR._hex))
        const prev = await sortedTrovesToken.getPrev(redemptionHints.firstRedemptionHint);
        const next = await sortedTrovesToken.getNext(redemptionHints.firstRedemptionHint);
        ...
        try {
            ...
            const tx = await troveManagerMain.redeemCollateral(
                ...
                status,
                new BigNumber(1e18).toFixed());
            setCurrentWaitInfo({ type: "loading", info: "Redeem " + Number(amount.toFixed(4)).toLocaleString() + " vUSD" })
    ...
    const Redeem = () => {
        ...
        if (!showRedeem) {
            tooltip.error({ content: "Redemption should be available in " + getCountDown(), duration: 5000 })
            return
        }
        if (!amount) {
            return
                ...
                <div className={`${styles.Redeem} ${"dappMain"}`}>
                    ...
                    <div className={styles.redeemMain}>
                        ...
                        <div className={styles.infoMain}>
                            <div className={amount && showRedeem ? "button rightAngle height " : 'button rightAngle height disable'} onClick={() => Redeem()}>Redeem</div>
                            ...
                            <div className={styles.data} style={{ "borderTop": "none" }}>
                                ...
                                <div className={styles.dataItem}>
                                    ...
                                    <span>${formatNum(expectedCollateralReceived * rosePrice)}</span>
                                </div>
                                <div className={styles.dataItem}>
